...
HelpWindow::HelpWindow(wxWindow* parent, std::function<void(const std::string&)> on_insert_quick)
  ...
  auto start = wxString(
      ...
      L"2. Команды сами не создаются — «Пресеты…» или «Добавить». Вкладки справа — группы («Группа+»); без группы — «Общее».\n"
      ...
      L"Окно\n\nСлева серверы, справа команды по группам-вкладкам, внизу лог. Колонка «Последний раз» — из журнала; «Ср. время» — среднее по всем запускам этой команды.\n"
      ...
      L"Связка (bundle) — последовательность команд выбранного VPS, в том числе из разных групп. Создайте связку, выберите команды и порядок, затем «Запустить связку»: укажите паузу в секундах между шагами. «Стоп» прерывает и текущую команду, и очередь. Ошибка на шаге останавливает связку.\n\n"
