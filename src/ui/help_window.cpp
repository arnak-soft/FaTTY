...
HelpWindow::HelpWindow(wxWindow* parent, std::function<void(const std::string&)> on_insert_quick)
  ...
  auto start = wxString(
      ...
      L"Порядок столбцов по умолчанию: Папка, Название, Команда, Комментарий, Последний раз. Заголовки перетаскиваются; клик по Название / Команда / Комментарий сортирует.\n"
