...
HelpWindow::HelpWindow(wxWindow* parent, std::function<void(const std::string&)> on_insert_quick,
  ...
  auto start = wxString(
      ...
      L"Порядок столбцов по умолчанию: Папка, Название, Команда, Комментарий, Последний раз, Ср. время. Заголовки перетаскиваются; клик по заголовку сортирует (повторный клик меняет направление). У активного столбца — стрелка и подсветка.\n"
      ...
      L"Выберите VPS → «Подключить». Работает параллельно с F5. Ctrl+V вставляет из буфера. "
      L"Галочка «Вывод команд сюда» направляет F5/связки в Shell (сбрасывается при каждом запуске FaTTY; "
      L"постоянно — Настройки → Общие).\n"
