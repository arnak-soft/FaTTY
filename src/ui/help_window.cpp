...
HelpWindow::HelpWindow(wxWindow* parent, std::function<void(const std::string&)> on_insert_quick)
  ...
  auto start = wxString(
      ...
      L"Окно\n\nСлева серверы, справа команды по папкам-вкладкам, внизу лог. Колонка «Последний раз» — из журнала; «Ср. время» — среднее по всем запускам этой команды.\n"
      ...
      L"Порядок столбцов по умолчанию: Папка, Название, Команда, Комментарий, Последний раз, Ср. время. Заголовки перетаскиваются; клик по Название / Команда / Комментарий / Ср. время сортирует.\n"
      L"Перед запуском — предупреждение. В правке команды можно снять галочку «Предупреждать перед запуском».\n"
      L"Несколько команд: Ctrl/Shift+клик, затем «Переместить в папку» или F5 — все выбранные в очередь.\n"
      L"«Сбросить в ~» возвращает рабочую папку в домашнюю.\n\n"
      L"Связка (bundle) — последовательность команд выбранного VPS, в том числе из разных папок. Создайте связку, выберите команды и порядок, затем «Запустить связку»: укажите паузу в секундах между шагами. «Стоп» прерывает и текущую команду, и очередь. Ошибка на шаге останавливает связку.\n\n"
